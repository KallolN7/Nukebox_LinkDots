...
namespace Hitcode_linkDots
{
    ...
    {// Singleton<GameData> {
        ...
        public static int[] totalLevel = { 100, 99, 50, 50, 50 };//total levels,currently,we make things easier,only use 50 levels the same for each difficulty
        ...
        public void init()
        {
            ...
            for (int i = 0; i < dotPoses.Count; i++)
            {
                ...
                int tindex = dotPoses[i]["v"][0]["y"] * GameData.bsize + dotPoses[i]["v"][0]["x"];
                GameData.instance.DotColorData[tindex] = i + 1;

                int tcount = dotPoses[i]["v"].Count;
                int _tx = GameData.instance.dotPoses[i]["v"][tcount - 1]["x"];
                int _ty = GameData.instance.dotPoses[i]["v"][tcount - 1]["y"];
                tindex = _ty * GameData.bsize + _tx;
                GameData.instance.DotColorData[tindex] = i + 1;
